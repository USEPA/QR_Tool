...
class RecycleLabel(Widget):
    ...
    def refresh_label(self, *args):
        lcls = CoreLabel
        label = lcls(text=self.text, text_size=(self.width, None),
                     halign=self.halign, font_size=self.font_size,
                     font_name=self.font_name, markup=self.markup)
        ...
        markup = self.markup
        color = self.color
        data = ({"text": " ".join([word.text for word in line.words]),
                 "font_name": font_name, "font_size": font_size, "height": line.h,
                 "size_hint_y": None, "halign": halign, "markup": markup, "color": color}
...
"""
    ...
    runTouchApp(RecycleLabel(text=LOREM))
